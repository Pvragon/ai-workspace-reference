...
- **Python environment** with `google-api-python-client` + `google-auth` installed. `team-lib/_admin/setup_workspace.sh` creates one and installs them. Resolve it once, portably, instead of hardcoding a layer path:

  ```bash
  # first venv that exists, else system python3
  WS="${PVRAGON_WORKSPACE:-$HOME/ai-workspace}"
  for c in "$WS/team-lib/.venv/bin/python" "$WS/my-lib/.venv/bin/python" "$(command -v python3)"; do
    [ -x "$c" ] && PY="$c" && break
  done
  ```
- **OAuth credentials** at `~/.google_workspace_mcp/credentials/<your-workspace-email>.json`
- **`GOOGLE_WORKSPACE_EMAIL`** set (or pass `--email`) — the script no longer assumes an address
...
```bash
"$PY" "$WS/team-lib/executions/list_shared_drives.py" --depth 1
```
...
```bash
"$PY" "$WS/team-lib/executions/list_shared_drives.py" --depth 1 --json
```
